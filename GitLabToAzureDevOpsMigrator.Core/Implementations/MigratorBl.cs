...
namespace GitLabToAzureDevOpsMigrator.Core.Implementations
{
    public class MigratorBl : IMigratorBl
    {
        ...
        public async Task CollectGitLabIssues(string gitLabUrl, int gitLabGroupId, int gitLabProjectId, string gitLabAccessToken)
        {
            ...
            var startingProcessMessage = $"Collecting GitLab issues, there are {allIssuesCount} issues to collect.";
            ...
            var endingProcessMessage = $"{Environment.NewLine}Collecting GitLab issues complete, {count} issues collected. Error collect {errorCount} issues.";
